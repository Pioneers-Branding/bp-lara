    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function($query){        
                $editBtn = '<a href="'.route('vendor.products-variant-items.edit', $query->id).'" class="btn btn-primary text-white ml-2 " style="margin-right:5px"><i class="far fa-edit"></i></a>' ;
                $deleteBtn = '<a href="'.route('vendor.products-variant-items.destroy', $query->id).'" class="btn btn-danger text-white ml-2 delete-item"><i class="far fa-trash-alt"></i></a>' ;                
            ...
            ->rawColumns(['action', 'status', 'is_default', 'variant_name'])
            ->setRowId('id');
    }
